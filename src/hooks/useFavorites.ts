...
export function useFavorites(): UseFavoritesReturn {
  const {
    favorites,
    setFavorites,
    addToFavorites: addToState,
    removeFromFavorites: removeFromState,
    clearAllFavorites: clearAllFromState,
  } = useAppState();

  // ローカルストレージからお気に入りを管理
  const { value: storedFavorites, setValue: setStoredFavorites } =
    useLocalStorage<DogImage[]>(LOCAL_STORAGE_KEYS.FAVORITES, []);
